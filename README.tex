...
\chapter{Agenda}
\label{sec:org7a61d96}

Quelques sujets que l'on pourrait aborder :

\begin{itemize}
\item mon parcours avec Emacs ;
\item le thème (officiel) de couleurs \hyperref[sec:org12b1e43]{Leuven} ;
\item ma configuration Emacs (\og \hyperref[sec:orgdc5cb03]{Emacs Leuven}\fg{}) ;
\item \hyperref[sec:orga1252c9]{org-html-themes} ;
\item \hyperref[sec:org52231b0]{boost-org-latex-toolkit} ;
\item \hyperref[sec:orgd483ceb]{mon organisation quotidienne} ;
\item \hyperref[sec:org8f3de76]{quelques astuces} et automatisations ;
\item les \hyperref[sec:org884bbd6]{formations EmacsBoost}.
\end{itemize}

Tous ces projets répondent à des besoins rencontrés dans mon travail quotidien.
\chapter{leuven-theme}
\label{sec:org12b1e43}
...
\section{VCS (Git)}
\label{sec:org9a94c89}

\subsection*{EDiff}
\label{sec:org019f170}
...
\emph{Hey user, this is an Org macro\ldots{}}
...
\label{sec:org44da841}

Quelques commandes permettent d'établir un pont pratique entre Emacs et le
terminal.

\begin{description}
\item[{\lstinline|e|}] Ouvrir un fichier dans l'instance Emacs en cours à l'aide de \verb|emacsclient|.

\begin{lstlisting}[language=shell,numbers=none]
  alias e='emacsclient -n -a emacs'
\end{lstlisting}

\item[{\lstinline|cate|}] Afficher dans le terminal le contenu du buffer actuellement actif dans
Emacs.

\item[{\lstinline|cde|}] Se placer dans le répertoire du fichier actuellement ouvert dans Emacs.
\end{description}
