...
- ~e~ :: Ouvrir un fichier dans l'instance Emacs en cours à l'aide de =emacsclient=.

  #+begin_src shell
  alias e='emacsclient -n -a emacs'
  #+end_src

- ~cate~ :: Afficher dans le terminal le contenu du buffer actuellement actif dans
  Emacs.

- ~cde~ :: Se placer dans le répertoire du fichier actuellement ouvert dans Emacs.
